...
def write_markdown(rows: list[dict], output_dir: Path) -> None:
    total_covered = sum(row["covered_wall_cells"] for row in rows)
    total_wall = sum(row["total_wall_cells"] for row in rows)
    weighted_coverage = total_covered / total_wall if total_wall else 0.0
    lines = [
        "# Viewpoint-Planning Summary",
        "",
        f"Completed reports: **{len(rows)}** across maps {', '.join(str(row['map_id']) for row in rows)}.",
        "",
        "| Aggregate metric | Result |",
        "|---|---:|",
        f"| Wall-cell-weighted coverage | {weighted_coverage:.1%} ({total_covered}/{total_wall}) |",
        f"| Mean coverage by map | {statistics.fmean(row['coverage_fraction'] for row in rows):.1%} |",
        f"| Mean stops | {statistics.fmean(row['num_stops'] for row in rows):.1f} |",
        f"| Mean routed tour length | {statistics.fmean(row['tour_length_m'] for row in rows):.2f} m |",
        f"| Mean planning time | {statistics.fmean(row['planning_time_s'] for row in rows):.2f} s |",
        f"| Invalid stops | {sum(row['invalid_stop_count'] for row in rows)} |",
        "",
        "## Per-map results",
        "",
    ...
    for row in rows:
        lines.append(
            f"| {row['map_id']} | {row['coverage_fraction']:.1%} | {row['num_stops']} | "
            f"{row['tour_length_m']:.2f} m | {row['planning_time_s']:.2f} s | "
            f"{row['invalid_stop_count']} |"
        )
    lines.extend(
        [
            "",
            "## Generated figure",
            "",
            "- `performance_by_map.png` — coverage, stop count, and routed tour length for the current planner on every evaluated map.",
        ]
    )
...
def main() -> None:
    ...
    print(f"Summarized {len(rows)} viewpoint-planning reports into: {output_dir}")
